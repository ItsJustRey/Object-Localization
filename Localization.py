    ...
    def calculate_global_x_y(v0_color, v1_color, v2_color):
        try:
            _global_xyz_pts = {'x': None, 'y': None, 'z': None, 'length0': 0, 'length1': 0, 'length2': 0,
                               'isCalculated': False}
            if ((len(v0_color['x']) > MIN_NUM_POINTS) and (
                    len(v1_color['x']) > MIN_NUM_POINTS)):  # and (len(v2_color['pts']) > MIN_NUM_POINTS)):
                _global_xyz_pts['length0'] = round((v0_color['x'][-1] - X_REF_POINT) * math.cos(V0_ANGLE) + (
                            v0_color['y'][-1] - Y_REF_POINT) * math.sin(V0_ANGLE))
                _global_xyz_pts['length1'] = round((v1_color['x'][-1] - X_REF_POINT) * math.cos(V1_ANGLE) + (
                ...
                return
            determinant = (
                        1 / ((math.cos(V0_ANGLE)) * (math.sin(V1_ANGLE)) - (math.sin(V0_ANGLE)) * (math.cos(V1_ANGLE))))
